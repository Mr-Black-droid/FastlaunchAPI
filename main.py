...
def create_order(order: LaunchOrder, user: str = Depends(get_current_user)):
    ...
    new_id = c.lastrowid
    conn.close()
    return {"order_id": new_id, "price": final_price, "message": "Mission scheduled successfully."}
...
@app.put("/orders/{order_id}", tags=["Missions"])
def update_order(order_id: int, order: LaunchOrder, user: str = Depends(get_current_user)):
    final_price = calculate_price(order.rocket_type, order.rideshare)
    conn = sqlite3.connect(DATABASE)
    c = conn.cursor()
    
    c.execute('''UPDATE orders 
                 SET customer=?, rocket=?, price=?, is_rideshare=? 
                 WHERE id=?''', 
              (order.customer_name, order.rocket_type, final_price, 1 if order.rideshare else 0, order_id))
    ...
    return {"message": "Mission updated", "order_id": order_id, "new_price": final_price}
...
def delete_order(order_id: int, user: str = Depends(get_current_user)):
    ...
    return {"message": f"Mission {order_id} scrubbed from manifest."}
